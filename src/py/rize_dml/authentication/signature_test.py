...
from .signature import sign_parameters_model, prepare_eip712_message, hash_parameters, recover_model_signer, Parameters
...
def test_sign_parameters_model(eth_account):
    ...
    signed_message = sign_parameters_model(eth_account, model, chain_id, contract_address, app_name, round_number)
    print(type(signed_message))
    assert isinstance(signed_message, SignedMessage), "should return a SignedMessage"

    message = prepare_eip712_message(chain_id, contract_address, app_name, round_number, hash_parameters(model))
