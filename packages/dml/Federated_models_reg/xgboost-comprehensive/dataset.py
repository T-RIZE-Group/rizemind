...
def train_test_split_v2(partition: Dataset, test_fraction: float, seed: int):
...
def partitioning(dataset, num_clients):
    all_partitions = []
    for i in range(num_clients):
        all_partitions.append(dataset.sample(frac=0.5, random_state = 42))
    return all_partitions

def load_partition(all_partitions, partition_id):
    return all_partitions[partition_id]
