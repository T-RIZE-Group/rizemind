...
def get_model_parameters(model: RandomForestRegressor) -> NDArrays:
    """Returns the parameters of a sklearn RandomForestRegressor model."""
    # params = [tree.tree_.__getstate__() for tree in model.estimators_]
    params = pickle.dumps(model)
    return params

def set_model_params(model: RandomForestRegressor, params: NDArrays) -> RandomForestRegressor:
    """Sets the parameters of a sklearn RandomForestRegressor model."""
    # for tree, param in zip(model.estimators_, params):
    #     tree.tree_.__setstate__(param)
    
    model = pickle.loads(params[0])

    return model

def set_initial_params(model: RandomForestRegressor):
    """Sets initial parameters for RandomForestRegressor."""
    # Create dummy data with random values to initialize the model
    n_features = 12
