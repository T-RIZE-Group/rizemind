...
def load_data(
    partition_id: int, num_partitions: int, batch_size: int, alpha: float = 0.1
):
    """
    ...
    counter = Counter()
    for sample in partition_train_test["train"]:
        label = sample["label"]
        counter[label] += 1
    logging.info(f"Client {partition_id} training sample counts: {dict(counter)}")
