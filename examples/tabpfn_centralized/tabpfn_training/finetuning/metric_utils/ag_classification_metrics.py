"""Classification metrics code from AutoGluon!

Ref: https://github.com/autogluon/autogluon/blob/master/core/src/autogluon/core/metrics/classification_metrics.py
"""
...
def pac(solution, prediction):
    ...
    if y_type == "binary":
        ...
        if len(solution.shape) == 2:
            if solution.shape[1] > 1:
                raise ValueError(
                    f"Solution array must only contain one class "
    ...
    return (pac - base_pac) / np.maximum(eps, (1 - base_pac))
...
def confusion_matrix(
    ...
    if (np.unique(labels)).size != n_labels:
