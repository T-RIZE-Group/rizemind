"""Code for metric functions copied from AutoGluon.

Minor modifications by removing some task types and moving some logic inside the functions.

Ref: https://github.com/autogluon/autogluon/blob/master/core/src/autogluon/core/metrics/__init__.py
"""
...
from finetuning.metric_utils import (
...
class Scorer(metaclass=ABCMeta):
    ...
    def _score(self, y_true, y_pred, **kwargs) -> float:
        y_true, y_pred, kwargs = self._preprocess(
            y_true=y_true,
            y_pred=y_pred,
            **kwargs,
        )
    ...
    def add_alias(self, alias):
        if alias == self.name:
            raise ValueError(
                f'The alias "{alias}" is the same as the original name "{self.name}". '
...
class _ThresholdScorer(Scorer):
    def _preprocess(self, y_true, y_pred, **kwargs):
        ...
        if y_type not in ("binary", "multilabel-indicator"):
            raise ValueError(
                f"{y_type} format is not supported in metric '{self.name}'",
            )
    ...
    def _preprocess(self, y_true, y_pred, **kwargs):
        ...
        if "quantile_levels" not in kwargs:
            raise AssertionError(
                "quantile_levels is required to score quantile metrics",
            )
...
def _add_scorer_to_metric_dict(metric_dict, scorer):
    ...
    for alias in scorer.alias:
        if alias in metric_dict:
            raise ValueError(
                f"Duplicated alias found! scorer={scorer}, alias={alias}. "
...
def make_scorer(
    ...
    **kwargs : additional arguments
    ...
    num_true = sum(
        [
            1 if needs else 0
            for needs in [needs_class, needs_proba, needs_threshold, needs_quantile]
        ],
    )
    if num_true > 1:
        raise ValueError(
...
def smape_func(y_true, y_pred) -> float:
    ...
    return np.average(
        np.abs(y_pred - y_true) / np.maximum(np.abs(y_true) + np.abs(y_pred), epsilon),
    )
...
quadratic_kappa = make_scorer(
    "quadratic_kappa",
    classification_metrics.quadratic_kappa,
    needs_class=True,
)
...
for name, metric in [
    ...
    for average in ["macro", "micro", "weighted"]:
        ...
        _add_scorer_to_metric_dict(
            metric_dict=BINARY_METRICS,
            scorer=globals()[qualified_name],
        )
        _add_scorer_to_metric_dict(
            metric_dict=MULTICLASS_METRICS,
            scorer=globals()[qualified_name],
        )
...
for name, metric, kwargs in [
    ...
    globals()[name] = make_scorer(
        name,
        partial(metric, average="macro", **kwargs),
        **scorer_kwargs,
    )
    macro_name = "{}_{}".format(name, "macro")
    globals()[name].add_alias(macro_name)
    _add_scorer_to_metric_dict(metric_dict=MULTICLASS_METRICS, scorer=globals()[name])
    averages = ["weighted"] if name == "roc_auc_ovo" else ["micro", "weighted"]
    for average in averages:
        qualified_name = f"{name}_{average}"
        globals()[qualified_name] = make_scorer(
            qualified_name,
            partial(metric, average=average, **kwargs),
            **scorer_kwargs,
        )
        _add_scorer_to_metric_dict(
            metric_dict=MULTICLASS_METRICS,
    ...
    if metric is not None and isinstance(metric, str):
        if problem_type is not None:
            if problem_type not in METRICS:
                raise ValueError(
                    f"Invalid problem_type '{problem_type}'. Valid problem types: {list(METRICS.keys())}",
                )
