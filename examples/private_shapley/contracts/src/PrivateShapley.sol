...
contract PrivateShapley is Ownable {
    ...
    mapping(uint256 => mapping(bytes32 => mapping(address => bool))) public trainerClaims;
    ...
    event CoalitionDataPublished(bytes32 indexed coalitionId, bytes32 bitfield, bytes32 merkleRoot);
     ...
     */
    function registerTrainer(address trainer) external onlyOwner returns (uint8) {
    ...
    function publishCoalitionData(
        ...
        require(!coalitionData[coalitionId].isPublished, "Coalition data already published");
    ...
    function claimReward(
        ...
        require(!trainerClaims[roundId][coalitionId][msg.sender], "Reward already claimed");
        
        // Check nonce hasn't been used in this round
        require(!nonceUsedInRound[roundId][nonce], "Nonce already used in this round");
        
        // Check if trainer is in the coalition bitfield (check if bit at trainerIndex is set)
        bool isInBitfield = (uint256(coalition.bitfield) >> (trainerIndex - 1)) & 1 == 1;
        ...
        require(verifyMerkleProof(merkleProof, coalition.merkleRoot, leaf), "Invalid Merkle proof");
     ...
     */
    function isTrainerInCoalition(bytes32 coalitionId, address trainer) public view returns (bool) {
