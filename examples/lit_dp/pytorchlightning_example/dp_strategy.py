...
class DPStrategy(SingleDeviceStrategy):
    ...
    def setup(self, trainer: pl.Trainer) -> None:
        ...
        elif isinstance(optimizers, (list, tuple)) and len(optimizers) == 1 and isinstance(optimizers[0], torch.optim.Optimizer):
            optimizer = optimizers[0]
        else:
            raise ValueError("Expected `configure_optimizers` to return a single optimizer instance.")
    ...
    def optimizer_step(self, optimizer: torch.optim.Optimizer, closure: Callable[[], Any], model: Optional[pl.LightningModule] = None, **kwargs: Any) -> Any:
