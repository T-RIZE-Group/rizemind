...
def test_distribute(token, aggregator, trainers):
    ...
    assert token.balanceOf(trainers[0]) == max_rewards, (
        "should have received max rewards"
    )
    assert token.balanceOf(trainers[1]) == (2 * max_rewards / 10**3), (
        "should not receive max rewards"
    )
    assert token.balanceOf(trainers[2]) == (50 * max_rewards / 10**6), (
        "should not receive max rewards"
    )
