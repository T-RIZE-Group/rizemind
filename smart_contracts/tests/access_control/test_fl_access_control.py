...
def test_is_aggregator(access_control, aggregator, trainers, non_trainer):
    assert access_control.isAggregator(aggregator) is True, (
        "aggregator role not assigned"
    )
    assert access_control.isAggregator(trainers[0]) is False, "trainers is aggregator"
    assert access_control.isAggregator(non_trainer) is False, (
        "non-trainer is aggregator"
    )
